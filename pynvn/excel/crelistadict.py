...
class credict:
    ...
    def reindexrownotnone(self):
        ...
        key_list = [cell.row for row in self.ws[self.rangea] for for cell in row if cell.value != None]
        """
        key_list = []
    ...
    def returnarrvaluebyindexcolumnandrow(self, valuecolumn):
        for valueco in valuecolumn:
            
            indexrcevalu = [[cell.row,cell.value] for rangecell in  self.ws[self.rangea] for cell in rangecell  if  cell.value in getvaluelist]
